...
import { parseJsonString } from '../shared/json'
puppeteer.use(StealthPlugin())

async function visitUpcomingMovies(page: Page) {
  let json = null
...
async function visitMovieLink(page: Page, link: string) {
  let json = null
  ...
  const response = await wait(5000)
...
async function visitMainPage(page: Page) {
  ...
  console.log(await page.evaluate(() => JSON.stringify(window.localStorage)))
...
export default async function (_req: Request) {
  ...
  try {
    ...
    // const content = await visitMainPage(page)
    ...
    // await filmstadenService.process(movies as any, showings as any)
